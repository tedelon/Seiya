...
def read_extra_files(cfg):
    ...
    passed as the ``extra_files`` parameter to :class:`seiya.core.InstallerBuilder`.
    """
...
def read_shortcuts_config(cfg):
    ...
    def _check_shortcut(name, sc, section):
        ...
        sc2 = dict(sc)
        # 'logo' is an alias for 'icon' and takes precedence if both are set.
        # Supports PNG, JPG, ICO formats (auto-converted to ICO at build time).
        if 'logo' in sc2:
            sc2['icon'] = sc2['logo']
        elif 'icon' not in sc2:
...
def get_installer_builder_args(config):
    ...
    # 'logo' is an alias for 'icon' and takes precedence if both are set.
    # Supports PNG, JPG, ICO formats (auto-converted to ICO at build time).
    args['icon'] = appcfg.get('logo', fallback=appcfg.get('icon', DEFAULT_ICON))
