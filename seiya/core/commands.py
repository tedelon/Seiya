...
def prepare_bin_directory(target, commands, bitness=32):
    for name, command in commands.items():
        ...
        # Commands live in bin/ subdirectory, so Python is one level up:
        #   $INSTDIR/Python/python.exe
        #   $INSTDIR/bin/<command>.exe  ->  ..\Python\python.exe
        # <launcher_dir> is NOT a token the distlib launcher resolves;
        # relative paths are resolved relative to the launcher exe's directory.
        if console:
